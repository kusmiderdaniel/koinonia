  ...
  if (selectedFieldIndex === 0) {
    return (
      <div className={`text-muted-foreground text-center ${isMobile ? 'text-xs py-2' : 'text-sm py-4'}`}>
        {isMobile ? (
          t('conditions.firstFieldMobile')
        ) : (
          <>
            {t('conditions.firstFieldDesktop')}
          </>
        )}
      </div>
    )
      ...
      {fieldConditions.length === 0 ? (
        <div className={`text-center ${isMobile ? 'py-2' : 'py-4'}`}>
          <p className={`text-muted-foreground ${isMobile ? 'text-xs' : 'text-sm'}`}>
            {isMobile ? t('conditions.noConditionsMobile') : t('conditions.noConditionsDesktop')}
          </p>
        </div>
      ) : (
