...
export async function getEventsForPicker() {
  ...
  const events: EventForPicker[] = (eventsResult.data || []).map((event: any) => ({
    id: event.id,
    title: event.title,
    ...
    campuses: (event.event_campuses || [])
      .map((ec: { campus: { id: string; name: string; color: string | null } | null }) => ec.campus)
      .filter((c: unknown): c is { id: string; name: string; color: string | null } => c !== null),
  }))
