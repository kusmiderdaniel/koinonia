-- Create a function to handle church creation with profile update atomically
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND church_id IS NOT NULL) THEN
  ...
  INSERT INTO churches (name, slug, email, phone, address, city, state, zip_code, invite_code, invite_code_generated_at)
  VALUES (p_name, p_slug, p_email, p_phone, p_address, p_city, p_state, p_zip_code, v_invite_code, NOW())
  RETURNING id INTO v_church_id;

  -- Update the user's profile to be owner of this church
  UPDATE profiles
  SET church_id = v_church_id, role = 'owner'
  WHERE id = auth.uid();
