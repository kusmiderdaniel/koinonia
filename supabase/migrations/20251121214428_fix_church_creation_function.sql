-- Recreate the church creation function with explicit column qualification
-- and RLS bypassing for cleaner execution
...
BEGIN
  ...
  -- Check if user already has a church (explicitly qualify column)
  IF EXISTS (
  ...
  -- This is safe because we've already validated the user
  ...
  -- Update the user's profile to be owner of this church
  UPDATE public.profiles
  SET
    profiles.church_id = v_church_id,
    profiles.role = 'owner'
  WHERE profiles.id = v_user_id;
...
COMMENT ON FUNCTION create_church_with_owner IS 'Creates a new church and assigns the calling user as owner. Uses SECURITY DEFINER to bypass RLS restrictions during creation. All column references are explicitly qualified to avoid ambiguity.';
