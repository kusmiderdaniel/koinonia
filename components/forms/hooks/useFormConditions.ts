...
import type { FormField, FormCondition, FormValues } from '../types'

interface UseFormConditionsOptions {
  fields: FormField[]
...
export function useFormConditions({
  ...
  conditions,
  values,
}: UseFormConditionsOptions) {
  const evaluateCondition = useCallback(
    (condition: FormCondition): boolean => {
      ...
      switch (condition.operator) {
        ...
        case 'is_not_any_of': {
          const disallowedValues: string[] = condition.value
            ? JSON.parse(condition.value)
            : []
          if (Array.isArray(sourceValue)) {
        ...
        case 'is_every_of': {
          const requiredValues: string[] = condition.value
            ? JSON.parse(condition.value)
            : []
          if (Array.isArray(sourceValue)) {
  ...
  return { visibleFields, evaluateCondition }
}
